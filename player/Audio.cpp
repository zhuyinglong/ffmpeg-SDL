...
double AudioState::get_audio_clock()
{
	...
	int bytes_per_sec = audio_ctx->sample_rate * audio_ctx->channels * 2;
...
void audio_callback(void* userdata, Uint8 *stream, int len)
{
	...
	while (len > 0)// 向设备发送长度为len的数据
	{
		if (audio_state->audio_buff_index >= audio_state->audio_buff_size) // 缓冲区中无数据
		{
			...
			{
				audio_state->audio_buff_size = 1024;
...
int audio_decode_frame(AudioState *audio_state, uint8_t *audio_buf, int buf_size)
{
	...
	audio_state->audio_clock += static_cast<double>(data_size) / (2 * audio_state->audio_ctx->channels * audio_state->audio_ctx->sample_rate);
