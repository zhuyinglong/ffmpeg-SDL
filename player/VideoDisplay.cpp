...
void video_refresh_timer(void *userdata)
{
	...
	if (video->stream_index >= 0)
	{
		...
		else
		{
...
#else
            schedule_refresh(media, 30);
#endif
			SwsContext *sws_ctx = sws_getContext(video->video_ctx->width, 
			...
			sws_scale(sws_ctx, 
				(uint8_t const * const *)video->frame->data, 
			...
			SDL_UpdateYUVTexture(video->bmp, &(video->rect),
				video->displayFrame->data[0], video->displayFrame->linesize[0],
				video->displayFrame->data[1], video->displayFrame->linesize[1],
				video->displayFrame->data[2], video->displayFrame->linesize[2]
			);
			SDL_RenderClear(video->renderer);
			SDL_RenderCopy(video->renderer, video->bmp, nullptr, nullptr);
